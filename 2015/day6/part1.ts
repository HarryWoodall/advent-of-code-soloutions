...
console.log(lights.filter((light) => light.isOn).length);
...
function executeCommand(command: Command) {
  ...
  switch (command.action) {
    case "turn on":
      lightsInRange.forEach((light) => (light.isOn = true));
      break;
    case "turn off":
      lightsInRange.forEach((light) => (light.isOn = false));
      break;
    case "toggle":
      lightsInRange.forEach((light) => (light.isOn = !light.isOn));
      break;
  }
}
