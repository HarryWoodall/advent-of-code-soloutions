...
async function createInput(day) {
  if (!fs.existsSync(`../input/${day}.txt`)) {
    ...
    const res = await fetch(`https://adventofcode.com/2021/day/${day}/input`, { headers: headers });
    ...
    if (
      ...
      fs.writeFileSync(`../input/${day}.txt`, text, (err) => {
        if (err) {
          console.error(err);
          return;
        }
...
function getInput(day) {
  if (fs.existsSync(`../input/${day}.txt`)) {
    return fs.readFileSync(`../input/${day}.txt`, "utf8", (err, data) => {
      if (err) {
        console.error(err);
      }
    });
